...
@pytest.mark.asyncio
async def test_runtime_safety_mode_blocks_new_lock_operations(client):
    ...
    blocked = await client.post("/v1/capacity/safety-block-user/lock", json={"amount": 10})
    ...
    lock = await client.post("/v1/capacity/safety-block-user/lock", json={"amount": 10})
    assert lock.status_code == 200
    assert lock.json()["total_locked_usdc"] == 10.0


@pytest.mark.asyncio
async def test_anchor_audit_trips_runtime_safety_mode_on_breach(client, db_session):
    await client.post(
        "/v1/security/runtime/safety-mode",
    ...
    audit = await client.post("/v1/security/runtime/anchor-audit?actor_id=anchor-monitor", json={})
    assert audit.status_code == 503
    assert "anchor breach" in audit.json()["detail"]

    state = await client.get("/v1/security/runtime/safety-mode")
