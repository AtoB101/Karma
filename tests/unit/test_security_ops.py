...
def test_build_security_ops_alert_report_detects_spikes():
    ...
    try:
        for _ in range(4):
            record_security_event(SecurityMonitoringEventType.FAILED_AUTH)
        for _ in range(3):
            record_security_event(SecurityMonitoringEventType.RATE_LIMIT_EXCEEDED)
        for _ in range(10):
            record_security_event(SecurityMonitoringEventType.VERIFY_REQUEST)
        for _ in range(4):
            record_security_event(SecurityMonitoringEventType.PRIVATE_RUNTIME_ERROR)
