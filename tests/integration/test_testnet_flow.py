"""
Karma — Integration Test: Mock Testnet Full Flow (KarmaBilateral)
...
def make_mock_bilateral():
    eng = MagicMock()
    eng.functions.tokenAllowed.return_value.call.return_value = True
    eng.functions.settle.return_value.build_transaction.return_value = {
        "from": "0xPayerAddress1234",
        "nonce": 5,
        "chainId": 11155111,
    }
    return eng
...
async def test_mock_testnet_full_flow():
    """receipts → bundle → verify → evidence_hash → bilateral settle (mocked)."""
    ...
    with patch("config.settings.settings.karma_bilateral_address", "0x" + "aa" * 20), patch(
        "config.settings.settings.erc20_token_address", "0x" + "bb" * 20
    ):
    ...
    assert tx_result.binding_id == 42
