...
**状态**：L1（静态包分发）、L2（节点层）、L3-1 里属于操作台的那半（核验页折叠成 3 步 +
服务商通道明确置灰）、L3-2（治理岗质押即开通 + 押金走则岗停）都已落地并进入验收闸门。
剩下 L4（只读节点）是路线图。
...
### L3-2 · 复核 / 仲裁：质押即开通（已落地 2026-09-23）

治理岗以前只有一条入口：进 `GOVERNANCE_VERIFIER_IDS` 白名单 —— 也就是「谁是复核员」
由平台说了算。这一层补上第二条入口，以及更重要的**在任判定**。

**开通**（`services/governance_stake.py`）：

- `GOVERNANCE_OPEN_JOIN=true` 之后，任何身份都能凭质押开 verifier / arbitrator 岗，
  下限由 `GOVERNANCE_MIN_STAKE_AMOUNT` 定（低于下限 422）。PUT 改 class / 改质押走同一把尺子；
- 但质押必须是**已经锁仓的 USDC**（`capacity.total_locked_usdc`）：没有锁仓背书 409。
  规矩与仲裁入池一致 —— 两条路用的是同一把尺子；
- 生产环境硬性禁止「开放申请 + 0 下限」和「开放申请 + 不要求锁仓背书」
  （`config/settings.py` 的生产校验）。少了这一条，开放申请就等于谁都能给自己开审批权；
- 白名单里的人**不受质押约束**：运维岗是平台自己的责任，不拿用户的押金来背书，
  也不因为别人的押金变动而停摆。

**在任** —— 这才是「质押」两个字能不能兑现的关键。治理岗有三档，别混着看：

| 档 | 怎么来的 | 跟着谁走 |
|---|---|---|
| 1 | 白名单 `GOVERNANCE_VERIFIER_IDS` | 名单。不跟押金 |
| 2 | 运维直接建档（`stake_amount = 0`） | 名单。不跟押金 |
| 3 | 质押开通（`stake_amount > 0`） | **押金：押金在则岗在，押金走则岗停** |

第 2 档在 API 上**开不出来**（没质押、没白名单就是 403），只可能是平台在服务端建的，
所以它不是一条绕开质押的后门。真正的变化在第 3 档 —— 岗不是「开一次管一辈子」：

五个治理入口（身份核验、主体认证复核、子身份 KYC、开发者实名复核、复核台队列）
每次动治理权限都拿承诺额与 `capacity.total_locked_usdc`**现算**一遍：

```
押金 ≥ 承诺额        →  岗在
押金低于承诺额        →  岗停（罚没 / 部分划走都算）
押金清零            →  岗停
```

现算意味着不需要维护一张「谁还在任」的状态表，也没有缓存可以过期 ——
这一轮测试里最关键的一条正是：押金划走之后，**同一个请求**上复核入口立刻 403，
而且状态机不往前走（那份 KYC 还是 pending，没被批掉）。

数据上只多了一列：`identity_role_profiles.stake_amount`（迁移
`0056_role_profile_governance_stake`）。它记的是「当时承诺押多少」，**不参与任何金额计算**
—— 钱的事实仍然只在链上与 capacity 镜像里。
...
3. **治理岗现在有两条路，但「白名单」那条还在。** 质押即开通（L3-2）已经落地：
   任何身份都能凭锁仓押金开 verifier / arbitrator 岗，押金一走岗就停。但平台**仍然可以**
   直接点名开岗（`GOVERNANCE_VERIFIER_IDS`），只是那条路不再是非走不可的了。
   彻底拿掉白名单 = 运维不再有任何例外通道，这一步还没做。
...
- `tests/integration/test_governance_stake.py`：10 项 —— 治理岗两条入口一把尺子
  （不带质押 / 低于下限 422、没有锁仓背书 409、改质押同样校验、白名单不受约束），
  以及「押金走则岗停」：押金被划走后 KYC 复核与复核台队列**当场** 403、状态机不往前走；
