...
1. Record execution steps into **ExecutionReceipt**-compatible payloads (`evidence_runtime/schemas.py`).  
...
2. Build **EvidenceBundle** + `proofHash` mapping (`evidence_runtime/evidence_adapter.py`).  
3. Call **structural verification** before proposing settlement (`evidence_runtime/verification.py`).  
4. Submit on-chain actions through **existing** `KarmaBilateral` flows (`docs/TESTNET_RUNBOOK.md`).
