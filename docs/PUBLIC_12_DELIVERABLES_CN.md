...
| # | 目标项 | 公开仓库落位 | 当前状态 |
|---|---|---|---|
| 1 | KarmaIdentitySBT 合约 | 概念映射到 `contracts/core/KYARegistry.sol`（身份/注册主路径） | 已有主路径 |
| 2 | KarmaVault 锁仓合约 | 概念映射到 `contracts/core/NonCustodialAgentPayment.sol`（`lockFunds/unlockFunds`） | 已有主路径 |
...
| 11 | 测试用例 | `tests/integration/test_api.py`, `tests/unit/test_sdk_adapters.py`, 合约测试 `contracts/test/*.t.sol` | 已落地 |
