...
7. admin 密钥 → 参数/费率/仲裁（多签部署断言 P1-5；freeze 时效已落地，上限 7 天）
...
- FROZEN → freeze_duration（≤7d）→ **自动解冻回冻结前状态，走既有超时路径**
...
3. Control Plane 按 registry 的 `audit_event` 与 `emergency_action` 接入监控与冻结（`services/security_control_plane.py`）。链上 `KarmaBilateral` 已接入 `CircuitBreaker` + timed freeze。**不登记 = 不被监控 = 不允许合入 main**（`security/registry/check_registry.py`）。
