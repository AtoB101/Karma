...
- **`view` + `block.timestamp`**: do **not** mark as `envfree` (e.g. `KYARegistry.verifyDID`); call with **`verifyDID(e, agent)`**, and declare **`optional`** in `methods` (do not use **`DISPATCHER`** on the contract-under-test).
