...
if [[ "$search_tool" == "rg" ]]; then
  git ls-files | rg -n "$blocked_paths_regex" >/tmp/security_guard_paths.txt || true
else
  git ls-files | grep -nE "$blocked_paths_regex" >/tmp/security_guard_paths.txt || true
fi
if [[ -s /tmp/security_guard_paths.txt ]]; then
...
if [[ "$search_tool" == "rg" ]]; then
  git ls-files | rg -ni "$blocked_name_regex" >/tmp/security_guard_names.txt || true
else
  git ls-files | grep -niE "$blocked_name_regex" >/tmp/security_guard_names.txt || true
fi
if [[ -s /tmp/security_guard_names.txt ]]; then
...
if [[ "$search_tool" == "rg" ]]; then
  git grep -nE 'sk-[A-Za-z0-9]{16,}|ghp_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}' -- . \
    | rg -v 'DO_NOT_COMMIT|<|replace_with|YOUR_|example|dummy|placeholder' >/tmp/security_guard_literals.txt || true
else
  git grep -nE 'sk-[A-Za-z0-9]{16,}|ghp_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}' -- . \
    | grep -vE 'DO_NOT_COMMIT|<|replace_with|YOUR_|example|dummy|placeholder' >/tmp/security_guard_literals.txt || true
fi
if [[ -s /tmp/security_guard_literals.txt ]]; then
